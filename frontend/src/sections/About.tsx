      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" ref={ref}>
        ...
        <SectionStroke className="p-5 sm:p-7 md:p-8 lg:p-10">
            ...
            {highlights.map((item, i) => (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="font-heading text-base sm:text-lg font-bold text-text-primary mb-2">{item.title}</h3>
          ...
          <div className="mt-8 sm:mt-10 md:mt-12 grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-5 md:gap-6">
            {stats.map((s) => (
              <div key={s.label} className="text-center p-4 sm:p-6 md:p-7 rounded-xl sm:rounded-2xl glass-card group hover:shadow-[0_0_25px_rgba(124,58,237,0.1)] transition-all duration-300">
                <div className="font-heading text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-primary-light to-accent bg-clip-text text-transparent group-hover:drop-shadow-[0_0_12px_rgba(124,58,237,0.5)] transition-all">
                  {s.value}
                </div>
                <div className="text-text-muted text-[11px] sm:text-xs md:text-sm mt-1 sm:mt-1.5 font-medium">{s.label}</div>
              </div>
            ))}
          </div>
        </SectionStroke>
